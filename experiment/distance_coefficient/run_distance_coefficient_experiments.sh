#!/bin/bash
# Lab4 Distance Coefficient Experiment Script
# Study the impact of different distance coefficients in adaptive routing
# 3D Torus 4x4x4, 64 CPUs, 64 dirs, adaptive routing (algorithm=4)

# Set results directory
results_dir="./results/distance_coefficient"
mkdir -p $results_dir

echo "Starting Lab4 Experiment: Impact of distance coefficient settings"
echo "Configuration: 4x4x4 3D Torus, 64 CPUs, 64 dirs, adaptive routing (algorithm=4)"
echo "Testing distance-coefficient values: -4, -2, 0, 2, 4"
...
# Distance coefficient values to test
# Negative: prefer short remaining distances (conservative, DOR-like)
# Zero: pure congestion-based routing
...
for dist_coeff in "${distance_coefficients[@]}"; do
    echo ""
    if [ $dist_coeff -lt 0 ]; then
        echo "Distance coefficient: $dist_coeff (conservative, DOR-like preference)"
    elif [ $dist_coeff -eq 0 ]; then
        echo "Distance coefficient: $dist_coeff (pure congestion-based routing)"
    else
        echo "Distance coefficient: $dist_coeff (load balancing preference)"
    fi
    echo "-----------------------------------"

    # Create result file for this combination
    result_file="$results_dir/${pattern}_distcoeff${dist_coeff}_results.csv"
    ...
    for rate in "${injection_rates[@]}"; do
        ...
        ./build/NULL/gem5.opt configs/example/garnet_synth_traffic.py \
            ...
            --vcs-per-vnet=4 \
            --escape-vcs=1 \
            --distance-coefficient=$dist_coeff \
            > /dev/null 2>&1
...
echo ""
echo "========================================"
echo "All distance coefficient experiments completed!"
...
for dist_coeff in "${distance_coefficients[@]}"; do
    echo "  ${pattern}_distcoeff${dist_coeff}_results.csv"
done
echo ""
echo "Distance Coefficient Meanings:"
echo "  Negative values (-4, -2): Conservative routing, prefer shorter paths (DOR-like)"
echo "  Zero (0): Pure congestion-based adaptive routing"
echo "  Positive values (2, 4): Load balancing, prefer longer paths to distribute traffic"
echo ""
echo "Use plot_distance_coefficient.py to generate distance coefficient comparison plots."

#!/bin/bash
# Lab4 Adaptive Tie-Breaking Experiment Script
# Study the impact of different tie-breaking strategies in adaptive routing
# 3D Torus 4x4x4, 64 CPUs, 64 dirs, adaptive routing (algorithm=4)

# Set results directory
results_dir="./results/adaptive_tie_breaking"
mkdir -p $results_dir

echo "Starting Lab4 Experiment: Impact of adaptive tie-breaking strategies"
echo "Configuration: 4x4x4 3D Torus, 64 CPUs, 64 dirs, adaptive routing (algorithm=4)"
echo "Testing tie-breaking strategies: x_first, uniform, z_first"
...
# Tie-breaking strategies to test
tie_breaking_strategies=("x_first" "uniform" "z_first")
...
for tie_strategy in "${tie_breaking_strategies[@]}"; do
    echo ""
    echo "Tie-breaking strategy: $tie_strategy"
    echo "-----------------------------------"

    # Create result file for this combination
    result_file="$results_dir/${pattern}_${tie_strategy}_results.csv"
    > $result_file  # Clear file
        ...
        for rate in "${injection_rates[@]}"; do
            ...
            ./build/NULL/gem5.opt configs/example/garnet_synth_traffic.py \
                ...
                --inj-vnet=2 --adaptive-tie-breaking=$tie_strategy \
                --vcs-per-vnet=4 \
                --escape-vcs=1 \
                > /dev/null 2>&1
...
echo ""
echo "========================================"
echo "All adaptive tie-breaking experiments completed!"
...
for tie_strategy in "${tie_breaking_strategies[@]}"; do
    echo "  ${pattern}_${tie_strategy}_results.csv"
done
echo ""
echo "Use plot_tie_breaking.py to generate tie-breaking strategy comparison plots."
